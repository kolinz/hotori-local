...
## Hotori の教育構造
Hotoriを用いた学習では、AIとの対話からデータセットを作成し、SFT（教師あり微調整）によって カスタムAIモデルを作成し配布するところまでを経験できます。

この過程を通じて、学習者は「自分でAIを作ることができた」という強い成功体験を得ることができます。

社会ではAIを利用する人は増え続けていますが、AIを育成し、モデルを作ることができる人材はまだ希少です。
Hotori は、AIを単に利用するだけではなく、AIを育て、作ることができる人材を育成するための学習環境を提供します。

```mermaid
flowchart TD

A[AIチャットで質問] --> B[AI回答を理解]
B --> C[良いQ&Aを選択]
C --> D[学習コレクションに保存]

D --> E[CSVデータセットとしてエクスポート]
E --> F[AIモデルの学習用データセット作成]

F --> G[AIモデルの学習実施（SFT/CPT）]
G --> H[カスタムAIモデル作成]
...
## ビルド・配布 (Mac)
ターミナルアプリ使って下記のコマンドを実行してください
```bash
npm run dist:mac
```
releaseフォルダの下に、.dngファイルが作成されます。このreleaseフォルダをzip形式等で圧縮して固めて配布できます。
